...
class LinearRegressor(Regressor):
    ...
    def evaluateMetric(self, z_batch, x_batch, batch_size, metric_type='RMSE', regularize=False):
        assert metric_type in self.__acceptable_metrics
        if metric_type == 'MSE':
    ...
    def updateParameters(self, z_batch, x_batch, batch_size, lr=0.01, loss_type='RMSE', regularize=False):  # dL / dw ?
        assert loss_type in self.__acceptable_metrics
        if loss_type == 'MSE':
            ...
            dw = -1 * np.dot((z_batch - self.getPrediction(x_batch)), x_batch) / \
                (batch_size * self.evaluateMetric(z_batch, x_batch, batch_size, 'RMSE', regularize))
